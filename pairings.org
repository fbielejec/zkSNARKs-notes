...
#+begin_src python
p = 23  # Characteristic
F = GF(p)  # Finite field of size 23
R.<x> = PolynomialRing(F)  # Polynomial ring over F
f = x^3 - x  # Define f(x) = x³ - x

# Compute f(x)^((p-1)/2) and extract the x^(p-1) coefficient
power = f^((p-1)//2)
hasse_invariant = power.coefficient(x^(p-1))
...
*** DONE P points order and the subgroup <P>
...
P_subgroup=[r*P for r in range(1,P.order()+1)]

print(f"Subgroup generated by P: {P_subgroup}")
#+END_SRC
...
*** IN-PROGRESS Fq field extension and point Q
#+BEGIN_SRC sage :session . :exports both
q=23
Fq=GF(q)
E=EllipticCurve(Fq, [-1,0])
