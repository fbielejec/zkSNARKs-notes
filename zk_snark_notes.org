...
Let $P,Q \in E(F_{q^k})[r]$ and let $D_P$ and $D_q$ be degree zero divisors with disjoint supportes such that $D_P~(P)-(\mathcal{O})$ and $D_Q~(Q)-(\mathcal{O})$. There exist funcitions f and g such that 

---
*NOTE*
Two divisors are said to be linearly equivalent if their difference is principal, i.e. they differ in a divisor of degree 0.
