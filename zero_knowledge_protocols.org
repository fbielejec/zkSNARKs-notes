...
** IN-PROGRESS R1CS for Edwards Addition on Tiny-jubjub (Exercise 99 and 100 MoonMath)
...
*Instance alphabet* in this case is the set of all possible pairs of points (affine coordinates):

$\Sigma_{I}: \hspace{.5em} \{ (x_{1},x_{2}) \times (x_{3},x_{4}): \hspace{.5em} x_{1},x_{2},x_{3},x_{4} \in F_{13} \}$

=witness alphabet=

*Witness alphabet* is the set of all possible points (affine coordinates):

$\Sigma_{W}: \hspace{.5em} \{ (x,y)): \hspace{.5em} x,y \in F_{13} \}$

=Decision function=

Maps an instance (a pair of points with coordinates in $F_{13}$) and a witness (a point) to /true/ if the instance points sum to the witness point on the Tinyy-jubjub curve:

$R_{add}: \hspace{.5em} ((F_{13} \times F_{13}), (F_{13} \times F_{13}))^{*} \times (F_{13} \times F_{13})^{*}  \rightarrow \{ true, false \};$
