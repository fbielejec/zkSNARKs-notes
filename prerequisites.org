...
* TODO <<EuclideanAlgorithm>>Euclidean Algorithm (and the Extended Euclidean Algorithm) 
* Modular inverse
...
A modular multiplicative inverse of a modulo \( m \) can be found by using the [[EuclideanAlgorithm][Extended Euclidean Algorithm]].
